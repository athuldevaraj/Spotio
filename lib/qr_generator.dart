...
class _QRCodeGeneratorPageState extends State<QRCodeGeneratorPage> {
  ...
  // Monitor QR collection and generate a new QR code if it becomes empty
  void _monitorQRCodeCollection() {
    _qrSubscription =
        _firestore.collection('qrCodes').snapshots().listen((snapshot) {
  ...
  Future<void> _initializeQRCode() async {
    ...
    try {
      final qrCodeDoc = _firestore.collection('qrCodes').doc('current');
  ...
  Future<void> _generateNewQRCode() async {
    ...
    final newQRValue = 'APP_QR_${timestamp}_$uniqueId';

    await _firestore.collection('qrCodes').doc('current').set({
      'qrValue': newQRValue,
      'timestamp': timestamp,
      'isValid': true,
    });
  ...
  Future<void> _markQRCodeAsUsed() async {
    try {
      ...
      // Delete the current QR code from Firestore
